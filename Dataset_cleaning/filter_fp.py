...
def load_annotations(file_path):
    ...
    with open(file_path, "r") as file:
        for line in file:
            ...
            if len(parts) < 5:
                continue
            # Make sure we are getting 6 values (class_id, x_center, y_center, width, height, confidence)
            if len(parts) == 5:
                # If confidence is missing, assume a default confidence value (e.g., 1.0 or 0.5)
...
def process_file_for_fp(args):
    """Process a single file to find false positive boxes."""
    ...
    false_positive_images = set()  # Use a set to avoid duplicate images
    for pred in pred_boxes:
        pred_class, *pred_box, pred_conf = pred
        
        # Use a different IoU threshold for hierarchical classes
        iou_threshold = low_iou_threshold if pred_class in hierarchical_classes else default_iou_threshold
        
        # Find the best IoU for the current predicted box
        best_iou = max((compute_iou(pred_box, gt_box[1:]) for gt_box in gt_boxes if gt_box[0] == pred_class), default=0)
        
        # If IoU is less than threshold and confidence > 0.5, mark it as a false positive
        if best_iou < iou_threshold and pred_conf > 0.5:
            false_positive_images.add(base_name)
    
    return list(false_positive_images)

def find_false_positives(gt_folder, pred_folder, image_folder, output_csv, hierarchical_classes, low_iou_threshold=0.4, default_iou_threshold=0.4):
    """Find false positives and save results to CSV using multiprocessing."""
    gt_files = [f for f in sorted(os.listdir(gt_folder)) if f.endswith(".txt")]
    
    with Pool(cpu_count() - 1) as pool:
        results = list(tqdm(pool.imap(process_file_for_fp, [(gt_file, gt_folder, pred_folder, image_folder, default_iou_threshold, hierarchical_classes, low_iou_threshold) for gt_file in gt_files]), total=len(gt_files), desc="Processing files"))
    
    # Flatten the results (list of lists) and get unique image names
    false_positive_images = set(item for sublist in results for item in sublist)
    
    # Save the unique image names to the CSV
    df = pd.DataFrame(list(false_positive_images), columns=["image_name"])
    df.to_csv(output_csv, index=False)
    print(f"False positive images saved to {output_csv}")
...
output_csv = "/home/sahithi_kukkala/sahithi/indicDLP/Dataset_cleaning/outputs/filtered_.8/fp_imgs_sample.csv"

# Define hierarchical classes
hierarchical_classes = {11, 14, 19, 20, 29, 33, 31, 35, 10, 26}

# Run the function to find false positives
find_false_positives(gt_folder, pred_folder, image_folder, output_csv, hierarchical_classes)
