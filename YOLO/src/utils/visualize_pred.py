...
def process_file(label_file):
    ...
    with open(label_path, "r") as f:
        for line in f:
            ...
            if len(parts) < 5:  # Now accepting files with 5 or 6 values (class_id, x_center, y_center, width, height, [confidence])
                continue
            ...
            # Get confidence score if available (6th element), otherwise set to None
            ...
            boxes.append((x_min, y_min, x_max, y_max, label_name, color, confidence))

    mask_with_text = img.copy()
    mask_without_text = img.copy()

    for x_min, y_min, x_max, y_max, label_name, color, confidence in boxes:
        ...
        if confidence is not None:
            display_text += f" {confidence:.2f}"  # Format confidence to 2 decimal places
        ...
        cv2.putText(mask_with_text, display_text, text_org, font, font_scale, (255, 255, 255), font_thickness, cv2.LINE_AA)
...
if __name__ == "__main__":
    ...
    print("Masked images with confidence scores saved successfully with 600 DPI!")